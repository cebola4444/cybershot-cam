...
void handleGallery() {
    ...
    server.sendContent(
        ...
        ".grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:8px;max-width:1100px;margin:0 auto}"
        ".card{background:#161616;border-radius:5px;overflow:hidden;border:1px solid #222}"
        ".card.ram{grid-column:1/-1}"
        "@media(min-width:600px){.card.ram{grid-column:span 2}}"
        ".thumb{width:100%;aspect-ratio:4/3;height:auto;object-fit:cover;display:block;cursor:pointer}"
        ...
        ".card.ram .thumb{aspect-ratio:4/3}"
