...
void handleEditor() {
    ...
    server.sendContent(
        ...
        "body{background:#0a0a0a;color:#ccc;font-family:monospace;padding:8px}"
        ...
        ".fn{text-align:center;font-size:10px;color:#444;margin-bottom:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"
        ".wrap{background:#111;margin-bottom:8px;min-height:80px}"
        "canvas{max-width:100%;display:block;margin:0 auto}"
        "#status{text-align:center;font-size:10px;color:#444;padding:4px 0}"
        ".ctrl{max-width:640px;margin:0 auto}"
        ".sec{font-size:9px;color:#333;text-transform:uppercase;letter-spacing:2px;margin:8px 0 4px;border-bottom:1px solid #1a1a1a;padding-bottom:2px}"
        ...
        ".glx button{color:#f80;border-color:#f80}"
        ".glx button.on{background:#f80;color:#000}"
    ...
    server.sendContent(
        ...
        "<div class='sec'>copy fx</div>"
        "<div class='row'><label>phase</label>"
        "<input type='range' id='sFase' min='0' max='628' value='0' step='1'>"
        "<span class='val' id='vFase'>0</span></div>"
        "<div class='row'><label>block</label>"
        "<input type='range' id='sBloco' min='16' max='256' value='96' step='8'>"
        "<span class='val' id='vBloco'>96</span></div>"
        "<div class='row'><label>freq</label>"
        "<input type='range' id='sFreq' min='20' max='400' value='100' step='10'>"
        "<span class='val' id='vFreq'>100</span></div>"
        "<div class='btns glx'><button id='bGlitch'>&#9889; copy fx: off</button></div>"
    ...
    server.sendContent(String(
        ...
        "let orig=null,origW=0,origH=0,rot=0,flipH=false,filt=null,glitchOn=false;"
        "const img=new Image();"
    ...
    server.sendContent(
        ...
        "function render(){"
          ...
          "}"
          // glitch generativo — copy() pattern (Processing-style)
          "if(glitchOn){"
            "const GW=c.width,GH=c.height;"
            "const g=+document.getElementById('sBloco').value;"
            "const t=+document.getElementById('sFase').value/100;"
            "const dv=+document.getElementById('sFreq').value;"
            "const base=document.createElement('canvas');"
            "base.width=GW;base.height=GH;"
            "base.getContext('2d').drawImage(c,0,0);"
            "ctx.save();ctx.translate(GW/2,GH/2);"
            "for(let y=-GH/2;y<GH/2;y+=g){"
              "for(let x=-GW/2;x<GW/2;x+=g){"
                "const cx=Math.floor(Math.max(0,Math.min(1,Math.tan(x/dv+t))))*g;"
                "const cy=Math.floor(Math.max(0,Math.min(1,Math.tan(y/dv+t))))*g;"
                "if(cx+g<=GW&&cy+g<=GH)"
                  "ctx.drawImage(base,cx,cy,g,g,x,y,g,g);"
              "}}"
            "ctx.restore();"
          "}"
        "}"
    ...
    server.sendContent(
        ...
        "wire('sFase','vFase');wire('sBloco','vBloco');wire('sFreq','vFreq');"
        ...
        "function setFilt(f){"
          "filt=(filt===f)?null:f;"
          "['bGray','bSepia','bInvert'].forEach(id=>document.getElementById(id).classList.remove('on'));"
          "const m={gray:'bGray',sepia:'bSepia',invert:'bInvert'};"
        ...
        "document.getElementById('bGlitch').onclick=()=>{"
          "glitchOn=!glitchOn;"
          "const b=document.getElementById('bGlitch');"
          "b.classList.toggle('on',glitchOn);"
          "b.innerHTML=glitchOn?'&#9889; copy fx: on':'&#9889; copy fx: off';"
          "render();};"

        "document.getElementById('bReset').onclick=()=>{"
          "['sBri','sCon','sSat','sVig','sCA'].forEach(s=>document.getElementById(s).value=0);"
          "['vBri','vCon','vSat','vVig','vCA'].forEach(v=>document.getElementById(v).textContent=0);"
          "document.getElementById('sFase').value=0;document.getElementById('vFase').textContent=0;"
          "document.getElementById('sBloco').value=96;document.getElementById('vBloco').textContent=96;"
          "document.getElementById('sFreq').value=100;document.getElementById('vFreq').textContent=100;"
          ...
          "filt=null;rot=0;flipH=false;glitchOn=false;psOn=false;psDir='h';psKey='luma';"
          "document.getElementById('bFlip').classList.remove('on');"
          "['bGray','bSepia','bInvert'].forEach(id=>document.getElementById(id).classList.remove('on'));"
          "document.getElementById('bGlitch').classList.remove('on');"
          "document.getElementById('bGlitch').innerHTML='&#9889; copy fx: off';"
